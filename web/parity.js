...
function initParity(){document.querySelectorAll('[data-icon]').forEach(el=>el.innerHTML=uiIcon(el.dataset.icon));const def=localStorage.getItem('work_default_currency');if(['₪','$'].includes(def))document.getElementById('modal-currency').value=def;document.querySelectorAll('#settings-dialog details').forEach(el=>el.addEventListener('toggle',()=>{if(el.open)document.querySelectorAll('#settings-dialog details').forEach(other=>{if(other!==el)other.open=false;});}));}
function openWebSettings(){document.getElementById('default-currency').value=localStorage.getItem('work_default_currency')||'₪';document.getElementById('settings-currency-icon').textContent=document.getElementById('default-currency').value;document.getElementById('settings-dialog').showModal();}
function saveWebSettings(){try{const value=document.getElementById('default-currency').value;localStorage.setItem('work_default_currency',value);if(editingShiftId===null)document.getElementById('modal-currency').value=value;document.getElementById('settings-dialog').close();renderShifts();showMessage('ההגדרות נשמרו');}catch{showMessage('לא ניתן לשמור את ההגדרות');}}
...
async function copyDisplayed(kind){const reportEntries=selectionMode?displayedEntries.filter(e=>selectedShiftIds.has(e.id)):displayedEntries;if(!reportEntries.length){showMessage('אין משמרות להעתקה');return;}const text=kind==='table'?WorkTransfer.csv(reportEntries,'\t'):reportEntries.map(e=>new Date(e.date).toLocaleDateString('he-IL')+' · '+e.category+' · '+e.hours+' שעות · '+e.currency+' '+e.totalEarnings.toFixed(2)+' · '+(e.isPaid?'שולם':'ממתין')).join('\n')+'\nסה״כ: '+formatTotals(WorkTransfer.totals(reportEntries));try{await navigator.clipboard.writeText(text);document.getElementById('report-dialog').close();showMessage('הדוח הועתק');}catch{showMessage('הדפדפן לא אפשר העתקה. אפשר לייצא קובץ דרך ההגדרות.');}}
function createShiftCard(entry,compact=false){
 ...
 if(entry.isGroupShift){try{const members=JSON.parse(entry.groupWorkersJson||'[]');const heading=document.createElement('h3');heading.textContent='עבודה קבוצתית';body.append(heading);let hours=0;for(const worker of members){hours+=Number(worker.hours);const line=document.createElement('p');line.textContent=worker.name+' · '+worker.hours+' שעות · '+(worker.isPaid?'שולם':'ממתין');body.append(line);}const total=document.createElement('p');total.textContent='כולל עובדים לתשלום: '+entry.currency+' '+(entry.totalEarnings+hours*(entry.employerRate??0)).toFixed(2)+' · השכר שלי כולל הפרש תעריפים: '+entry.currency+' '+(entry.totalEarnings+hours*((entry.employerRate??0)-(entry.workerRate??0))).toFixed(2);body.append(total);}catch{const p=document.createElement('p');p.textContent='לא ניתן להציג את פרטי הקבוצה. הנתונים נשמרו ללא שינוי.';body.append(p);}}
 const actions=document.createElement('div');actions.className='journal-card-actions';for(const [name,label,action] of [['check',entry.isPaid?'סמן כממתין':'סמן כשולם',()=>togglePaid(entry.id)],['edit','עריכה',()=>editShift(entry.id)],['trash','מחיקה',()=>deleteShift(entry.id)]]){const button=document.createElement('button');button.className='btn-mini';button.innerHTML=uiIcon(name)+'<span>'+label+'</span>';button.onclick=action;actions.append(button);}body.append(actions);card.append(summary,body);return card;
}

let selectionMode=false,selectedShiftIds=new Set();
function enterSelection(){selectionMode=true;renderShifts();}
...
function applySelection(action){
 ...
 if(action==='delete'&&!confirm('למחוק את '+selectedShiftIds.size+' המשמרות שנבחרו? פעולה זו אינה ניתנת לביטול.'))return;
 try{persistAll(WorkSelection.apply(shifts,selectedShiftIds,action));exitSelection();showMessage(action==='delete'?'המשמרות שנבחרו נמחקו':'מצב התשלום עודכן');}catch{showMessage('השמירה נכשלה. הנתונים לא שונו.');}
}
